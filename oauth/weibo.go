...
func GetAuthorizationUrl(appKey, redirectUrl string) string {
	...
	return OAuthURL + "/authorize?" + v.Encode()
}

func GetAccessToken(appKey, appSecret, redirectUrl, code string) (*SinaAccessToken, error) {
	v := url.Values{}
	v.Add("client_id", appKey)
	...
	if resp, err = http.PostForm(reqUrl, v); err == nil && resp.StatusCode == http.StatusOK {
		...
		var accessToken SinaAccessToken
...
func (s *Sina) weiboApi(method string, base string, query url.Values, v interface{}) (bool, error) {
	...
	if err != nil {
		beego.Info("fetch url %s %s", url1, err)
		panic(err)
	}
	defer resp.Body.Close()
...
func (s *Sina) GET(base string, query url.Values, v interface{}) bool {
	...
	if !ok && err != nil {
		beego.Info("Weibo GET API Error:", base, query)
...
func (s *Sina) POST(base string, query url.Values, v interface{}) bool {
	...
	if !ok && err != nil {
		// code := err.(WeiboError).Error_Code
		beego.Info("Weibo POST API Error:", base, query)
		msg := err.(WeiboError).Err + " " + err.(WeiboError).Request
		ReportFatalError(msg)
	}
	return ok
